...
import static com.example.android.testing.uiautomator.BatteryKPIAndroidP.BatteryKPIBefore.openWebTest_time;
 ...
 * 该类是主要的测试类, BatteryKPIBefore 类是用来在多次循环测试时进行输出文件的预备(删除可能在之前测试生成的文件等操作)
...
public class BatteryKPIAndroidP0 {
...
//            mDevice.wakeUp();
    ...
    public void test_001() {
        Log.d(TAG, "test_001 call call_count==" + BatteryKPIBefore.call_count);
        boolean isEnd = false;
        ...
        for (int i = 0; i < BatteryKPIBefore.call_count; i++) {
            try {
                mDevice.executeShellCommand(dialCMD + BatteryKPIBefore.CooperatingPhoneNum);
                setSpeakerphoneOn(true);
                sleep(2 * 1000);
                mDeviceSleep();
                sleep(BatteryKPIBefore.call_times * onemins + 5 * 1000);
                mWeakUp();
                sleep(1 * 1000);
...
//        wk.acquire();
        ...
        int endTime = (int) System.currentTimeMillis();
        while ((endTime - startTime) <= BatteryKPIBefore.playgames_time * onemins) {
...
//        wk.acquire();
        ...
        while (BatteryKPIBefore.weChatcount > 0) {
            BatteryKPIBefore.weChatcount--;
            try {
                mWeakUp();
...
//        wk.acquire();
        ...
        int startTime = (int) System.currentTimeMillis();
        while (BatteryKPIBefore.sendSMSCount > 0) {
            BatteryKPIBefore.sendSMSCount--;
            try {
                mWeakUp();
                sleep(2 * 1000);

                mDevice.executeShellCommand(String.format("am start -a android.intent.action.SENDTO -d sms:%s --es sms_body %s", BatteryKPIBefore.CooperatingPhoneNum, BatteryKPIBefore.smsMsg));
                sleep(2000);
                ...
                mDeviceSleep();
                sleep(BatteryKPIBefore.smsInterval * onemins);
            } catch (InterruptedException e) {
                e.printStackTrace();
            } catch (UiObjectNotFoundException e) {
                e.printStackTrace();
...
//        }
    ...
    public void test_005() {
        Log.d(TAG, "test 005 qq qqCount====" + BatteryKPIBefore.qqCount);
        boolean isEnd = false;
//        PowerManager pm = (PowerManager) applicationContext.getSystemService(Context.POWER_SERVICE);
//        PowerManager.WakeLock wk = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, TAG);
//        wk.acquire();
        ...
        while (BatteryKPIBefore.qqCount > 0) {
            Log.d(TAG, "qq count:" + BatteryKPIBefore.qqCount);
            BatteryKPIBefore.qqCount--;
            try {
                mWeakUp();
                ...
                edit.setText(BatteryKPIBefore.qqMsg);
                ...
                mDeviceSleep();
                sleep(BatteryKPIBefore.qqIntervals * onemins);
            } catch (InterruptedException e) {
                e.printStackTrace();
            } catch (UiObjectNotFoundException e) {
                e.printStackTrace();
...
//        wk.acquire();
        ...
        int endTime = (int) System.currentTimeMillis();
        while (endTime - startTime < BatteryKPIBefore.weiboTime * 60 * 1000) {
            try {
                endTime = (int) System.currentTimeMillis();
                mDevice.swipe(300, 600, 300, 200, 10);
                sleep(20 * 1000);
...
//        }
    ...
    public void test_008() {
        ...
        try {
            ...
            Log.d(TAG, "BatteryKPIBefore.MusicPackageName ===" + BatteryKPIBefore.MusicPackageName);

            startApp(BatteryKPIBefore.MusicPackageName, timeOut);
            sleep(5 * 1000);
            mDevice.wakeUp();
            if (BatteryKPIBefore.MusicPackageName.equals(googledMusicPackageName)) {
            ...
            } else if (BatteryKPIBefore.MusicPackageName.equals(androidMusicPackageName)) {
...
//            wk.release();
            sleep(20 * 1000);
            mDeviceSleep();
            sleep(BatteryKPIBefore.playMusic_time * onemins);
            mWeakUp();
            sleep(5 * 1000);
//            mDevice.pressHome();
            ...
            sleep(2 * 2000);
            closeAPP(BatteryKPIBefore.MusicPackageName);
//            mDeviceSleep();
//            wk.release();
...
//        wk.acquire();
        ...
        try {
            ...
            startApp(BatteryKPIBefore.FilesPackageName, timeOut);
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        try {
            ...
            if (BatteryKPIBefore.FilesPackageName.contains("files")) {
                ...
                //15分钟的视频，循环播放4次
                Log.d(TAG, "init playVideo_times: " + BatteryKPIBefore.playVideo_times);
                for (int i = 0; i < (BatteryKPIBefore.playVideo_times); i++) {
                    mWeakUp();
                    flag++;
                    Log.d(TAG, "playVideo_times: " + BatteryKPIBefore.playVideo_times);
                ...
                closeAPP(BatteryKPIBefore.FilesPackageName);
            }
            //如果预置的是filemanager
            else if (BatteryKPIBefore.FilesPackageName.contains("filemanager")) {
...
//        wk.acquire();
        ...
        if (backshutterButton.exists()) {
            int i;
            for (i = 0; i < BatteryKPIBefore.takePicture_count; i++) {
                try {
...
//        wk.acquire();
        ...
        try {
            ...
            if (videoshutterButton1.exists()) {
                ...
                sleep(BatteryKPIBefore.takeVideo_time);
                videoshutterButton1.clickAndWaitForNewWindow();
            }
                ...
                if (videoshutterButton.exists()) {
                    ...
                    if (videoBtn.exists()) {
                        ...
                        if (playBtn.exists()) {
                            ...
                            sleep(BatteryKPIBefore.takeVideo_time + 5 * 1000);
                        }
                    }
...
//        wk.acquire();
        ...
        try {
            ...
            for (int j = 0; j < BatteryKPIBefore.sendmail_count; j++) {
                ...
                sleep(2 * 1000);
                String input = BatteryKPIBefore.targetGmail;
                sleep(2 * 1000);
...
//        wk.acquire();
        ...
        Log.d(TAG, "standby time====" + BatteryKPIBefore.bootThenStandby_time * onemins);


        try {
            mDeviceSleep();
            sleep(BatteryKPIBefore.bootThenStandby_time * onemins);
//        wk.release();
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
...
//        }
//        isEnd = true;
//        assertTrue("test 014 end" , isEnd);
//        Log.d(TAG, "test 014 assertTrue end");
//    }
