...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def start_test(self):
        ...
        (table_name_1, date) = self.comboBox_list.currentText().split(r'     更新时间： ')

        apn_list = self.conversion_apn(apn_path)
        # print('anp list=====', apn_list)

        for i in range(0, len(apn_list)):
            # print('table_name_1=========', table_name_1, '      apn list=========', apn_list[i])
            select_database_result = self.select_database(table_name_1, apn_list[i])
            # print('select_database_result==========', len(select_database_result))
    ...
    def start_test_2(self):
        ...
        if not self.comboBox_list_2.currentText():
            ...
            (table_name_2, date) = self.comboBox_list_2.currentText().split(r'     更新时间： ')
    ...
    def connect_database(self):
        try:
            global conn
            global cursor
            conn = sqlite3.connect('//192.168.3.88/sql/APNCHECK/APNTEST.db')
            cursor = conn.cursor()
        except:
            tk.messagebox.showerror('ERROR', '服务器链接错误！')
            return
    ...
    #select 数据库
    def select_database(self, table_name, list1):
        global conn
        global cursor
        try:
            sql_str = '''SELECT * from {0} where MCCMNC=\'{1}\' and Account_Name=\'{2}\''''.format(table_name, list1[2], list1[0])
            cursor.execute(sql_str)
            select_result = cursor.fetchall()
            if len(select_result) > 1:
                sql_str1 = '''SELECT * from {0} where MCCMNC=\'{1}\' and Account_Name=\'{2}\' and APN=\'{3}\'''' \
                    .format(table_name, list1[2], list1[0], list1[1])
                cursor.execute(sql_str1)
                select_result = cursor.fetchall()
                if len(select_result) > 1:
                    if not list1[-1] == '':
                        sql_str2 = '''SELECT * from {0} where MCCMNC=\'{1}\' and Account_Name=\'{2}\' and APN=\'{3}\' and Mnvo_match_data=\'{4}\'''' \
                            .format(table_name, list1[2], list1[0], list1[1], list1[-1])
